...
<section>
	<div class="container">
		...
		<div class="row active_rfp">
			<div class="col-md-12 firrst_ul">
				<h4>Active Requests
					...
					<a class="btn btn-success pull-right" onclick="show_modal_func()" >
						<i class="fa fa-plus"></i>Procedure Search
					</a>
				</h4>	
			</div>	
		...
		<div class="divider divider-color divider-center divider-short">
			<!-- divider -->
			<i class="fa fa-cog"></i>
			<img src="<?= DEFAULT_IMAGE_PATH ?>favicon.png">
		</div>
			...
			<form action="<?=base_url('dashboard/choose_appointment_schedule')?>" method="POST" id="frm_choose_app">
				...
				<div class="modal-body">
					<div class="row">
						...
						<div class="col-sm-12 desc-text desc-text-fix">
							<h5><span>Please, select and confirm your appointment:</span><img class="arrow-down-app" src="<?=DEFAULT_IMAGE_PATH.'arrow-down.png';?>"></h5>
						</div>
...
<!-- ==================== Modal Popup For Refund Payment ========================= -->
<div class="modal fade procedure_search_class" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog modal-md">
		<div class="modal-content">
			...
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title" id="myLargeModalLabel">Procedure Search</h4>
			</div>
			<!-- body modal -->
			<div class="modal-body">
				<div class="row">
					<div class="col-sm-12">
						<div class="form-group">
							...
							<div class="search-div-fix">
							<input type="text" placeholder="Search" class="form-control" id="search_keyword_treatment_category"
								   onkeydown="search(this)">
							<!-- <input name="submit" class="btn btn-info" value="Search" onclick="search_treatment_category()"> -->
							<a  onclick="search_treatment_category()" class="for_pointer">
					...
					<span style="display: block;clear: both;text-align: center;">						
						<b style="text-align: center; display: none;" id="no_data_found">No data Found</b>
					</span>

					<div class="col-sm-12 search-content">
						<ul id="search_category_ul">
...
<script type="text/javascript" >
	...
	?> 
		setTimeout(function(){ 
			$('html, body').animate({ scrollTop: $('.patient-rfp-list').offset().top -70 }, 1000);
			bootbox.alert("You currently have Request(s) in draft status. Go in your DASHBOARD to the section 'Your Quote Requests'"); 
			$.popVar("show_popup");
		}, 2000);
	<?php } ?>
	...
	function show_modal_func(){
		$('.procedure_search_class').modal('show');
	}

	function search_treatment_category(){
		
		var keyword = $('#search_keyword_treatment_category').val();

		$.ajax({
            url:'<?php echo base_url()."dashboard/search_result"; ?>',
            method:'POST',
            data:{keyword:keyword},
            success:function(data){            	
            	$('#no_data_found').show();
            	$('#search_category_ul').html('');
            	if(data.length > 1){
            		$('#no_data_found').hide();
            		$('#search_category_ul').html(data);
            	}
            }
        });		
	}

	function redict_to(is_blocked,treatment_cat_id){

		switch(is_blocked){
			case 1: window.location.href='<?php echo base_url()."rfp/add"; ?>';
					break;
			case 2: window.location.href='<?php echo base_url()."dashboard/quick_rfp_form?cat_id="; ?>'+treatment_cat_id;
					break;
			case 3: '';
					break;			
		}
