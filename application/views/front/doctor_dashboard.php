		...
		<div class="modal-content">
			...
			<form action="<?=base_url('rfp/make_doctor_payment')?>" method="POST" id="form_doctor_payment"
				...
				<div class="modal-footer">
					<div class="col-sm-12">
						<div class="form-group">
							<!-- Hidden Field for orginal price -->
							<input type="hidden" name="h_due_1" id="h_due_1_id">
							<input type="hidden" name="h_due_2" id="h_due_2_id">							
							<input type="hidden" name="h_rfp_id" id="h_rfp_id_frm">
							<input type="hidden" name="h_total_due_modal" id="h_total_due_modal">
							<input type="hidden" name="h_orignal_price" id="h_orignal_price">
							<input type="hidden" name="h_is_coupon_applied" id="h_is_coupon_applied" value="0" >
							<!-- Hidden Field for orginal price -->
							...
							<input type="submit" name="submit" class="btn btn-info" value="Proceed">
					...
					<div class="row">
						...
						<div class="col-sm-12">
							<div class="form-group">
								...
								<a class="btn btn-info" onclick="choose_app_option()"><i class="fa fa-calendar"></i> Share available Appointments</a>
							</div>	
						</div>	
					</div>	
				</div>
...
<script type="text/javascript">
	...
	function show_modal_doctor(obj){
		...
		// Hidden Field for orginal price
		$("#h_due_1_id").val(due_1);
		$("#h_due_2_id").val(due_2);
		$("#h_rfp_id_frm").val(rfp_id);
		$("#h_total_due_modal").val(total_due);
		$("#h_orignal_price").val(total_payment);
		$("#h_is_coupon_applied").val('0');
		// End Hidden Field for orginal price
	...
	$('#coupan_code').keypress(function(e){
	    if(e.which == 13){//Enter key pressed
	        e.preventDefault();
	        $('#apply-code').click();//Trigger search button click event
	    }
			...
			$.post("<?=base_url('rfp/fetch_coupan_data')?>",{'coupan_code' : coupan_code},function(data){
				...
				if(data != '' && data['id'] != null) {	 
					...
					if(data['per_user_limit'] > data['total_apply_code']){
						...
						var total = parseFloat($('#h_total_due_modal').val());
	...
	function calculate_original_price(){
		$('.due_1_price').html('$ '+$("#h_due_1_id").val());
		$('.due_2_price').html('$ '+$("#h_due_2_id").val());
		$('#due_1_id').val($("#h_due_1_id").val());
		$('#due_2_id').val($("#h_due_2_id").val());
		$('#orignal_price').val($("#h_orignal_price").val()); // orignal price
		$('#total_due_modal').val($("#h_total_due_modal").val()); // after discount price
		$('.total-price').html('$ '+ $("#h_total_due_modal").val());
	}
	//--------------- End Calculate Original Price ---------------------------
