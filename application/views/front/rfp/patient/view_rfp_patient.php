<style>
	.coupan-msg-new{ padding-left: 15px;  }
</style>
					...
					<div class="panel-body">
						...
						<div class="rfp-basic">
							...
							<div class="col-sm-6">
								...
								<span>
									<?php
										echo date_diff(date_create($record['birth_date']), date_create('today'))->y . " years";
	...
	<div class="modal-dialog modal-md">
		<div class="modal-content">
					...
					<div class="row">
						...
						<div class="">
							<div class="form-group">
								...
								<br/>
								<span class="coupan-msg-new"></span>	
						...
						<div class="col-sm-12">
							<div class="form-group">
								<h4>Final Price : $ <span class="final-prices"><?=$rfp_price?></span></h4>
							</div>
						</div>						
					</div>	
				</div>
			...
			<div class="modal-footer">
				<div class="col-sm-12">
					<div class="form-group">
						<input type="submit" name="submit" class="btn btn-info make-payment" value="Proceed" onclick="formSubmit(this)" id="newFormId">
...
<script>
	...
	<?php	    
		...
		if($show_popup != ''){		
	?> 
		setTimeout(function(){ $('.promotional_code').modal('show'); $.popVar("show_popup"); }, 2000);
	<?php } ?>
			...
			$.post("<?=base_url('rfp/fetch_coupan_data')?>",{'coupan_code' : coupan_code},function(data){
				...
				if(data != '' && data['id'] != null){
					...
					if(data['per_user_limit'] > data['total_apply_code']){
						discount_amt = 	((<?=$rfp_price?> * data['discount'])/100);
						var final_price = <?=$rfp_price?> - discount_amt;
						$(".final-prices").html(final_price);
						$(".coupan-msg").html("Coupon Code Apply Successfully");
						$(".coupan-msg").css("color", "green");
					}
					else{
						var final_price = <?=$rfp_price?>;
						$(".final-prices").html(final_price);
	...
	function formSubmit(){
		...
		$('#newFormId').attr('disabled',true);
		$('.coupan-msg-new').html('We are processing your request.....');
		...
		setTimeout(function(){
			$('#newFormId').attr('disabled',false);
			$('.coupan-msg-new').html('');

			if($('.coupan-msg').html() != 'Invalid Coupon Code'){
				$('#frmcoupan').submit();				
			}
		}, 6000);
