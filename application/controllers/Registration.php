...
class Registration extends CI_Controller {
    ...
    public function user(){
        ...
        if($this->form_validation->run() == FALSE){
            if($this->session->flashdata('reg_back') == ''){
            ...
            if($user_data){
                ...
                $this->session->unset_userdata('reg_data');
                $this->session->set_flashdata('login_email',$this->input->post('email_id'));
                if($user_data['is_blocked'] == '1' || $user_data['is_deleted'] == '1'){
                    // $msg = 'The Email is already in use - please, use the login screen. If you need to reset your password check here <a href="faq#q3"> FAQ </a> Password reset';
                    $msg = 'The Email is already in use - please, reach out to customer service, to re-enable your account <a href="/contact_us"> CONTACT US </a>';
                }else{
                    $msg = 'You already have an account under the entered Email address.';
                }
                ...
                $r_data = $this->db->get_where('users',['email_id' => $this->input->post('email_id')])->row_array();
                $reg_data['email_id'] = $this->input->post('email_id');
                $reg_data['zipcode'] = $this->input->post('zipcode');
                $reg_data['state_id'] = $this->input->post('state_id');
                $reg_data['role'] = $this->input->post('role');

                $this->session->set_userdata('reg_data',$reg_data);

                if(!empty($r_data)){
                    if($r_data['is_deleted'] == '1'){
                        // $msg = 'The Email is already in use - please, use the login screen. If you need to reset your password check here <a href="faq#q3"> FAQ </a> Password reset';
                        $msg = 'The Email is already in use - please, reach out to customer service, to re-enable your account <a href="/contact_us"> CONTACT US </a>';
                        $this->session->set_userdata('new_error',$msg);
                    }
                }

                $data['subview']='front/registration/registration_user_2';
                $this->load->view('front/layouts/layout_main',$data);     
            }
        } 
    }
    ...
    public function back_user(){
        $this->session->set_flashdata('reg_back','1');
    ...
    public function user_2(){
        ...
        $this->form_validation->set_rules('zipcode', 'zipcode', 'callback_validate_zipcode',['validate_zipcode'=>'Please, verify your ZIP Code.']);
        ...
        if($this->form_validation->run() == FALSE){
            if($this->input->post('email_id') == '') // IF direct access 2nd step then goto first step 
            {
                $this->session->unset_userdata('reg_data');
                redirect('registration/user');
            }else{
    ...
    */
    public function verification($id='0'){
        ...
        if($this->Users_model->CheckActivationCode($id))
        {
            ...
            $user_data = $this->db->get_where('users',['activation_code'=>$id])->row_array();
            ...
            $quick_rfp_data = $this->db->get_where('rfp',['rfp_mail'=>$user_data['email_id']])->result_array();

            if( (!empty($quick_rfp_data))){

                // It will assign RFP patient id to user id only if user has patient role
                if($user_data['role_id'] == 5){
                    $this->db->update('rfp', ['patient_id'=>$user_data['id']], ['rfp_mail'=>$user_data['email_id']]); // Set Patient id to logged in user id
                    $this->db->update('rfp', ['rfp_mail'=>NULL], ['rfp_mail'=>$user_data['email_id']]); // Make RFp patient mail NULL
                    ...
                    if(count($quick_rfp_data) > 1){
                        redirect('dashboard?show_popup=yes');
                    }else{    
                        redirect('rfp/view_rfp/'.encode($quick_rfp_data[0]['id']).'?show_popup=yes');
                    }
                }else{
                    $this->db->delete('rfp',['rfp_mail'=>$user_data['email_id']]);
                }
            }
    ...
    */
    ...
    function testemail(){
        ...
        $this->email->from('dhk@narola.email', 'Dhaval');
        $this->email->to('had@narola.email');
