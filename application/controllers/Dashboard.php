...
class Dashboard extends CI_Controller {
    ...
    public function __construct(){
        ...
        if(!isset($this->session->userdata['client'])){
            ...
            if($this->session->userdata['client']['role_id'] == 4) {
                ...
                $protected_methods = array('save_dashboard_alert','delete_appointment',
                                            'reschedule_appointment','change_filter_notify_status',
                                            'delete_search_filter','save_map_address');
            }
    ...
    public function search_result(){
        ...
        $res = $this->db->get('treatment_category')->result_array();
        
        $new_str = '';        

        if(!empty($res) && !empty($keyword)){
            foreach($res as $r){
                $new_str .= '<li><a onclick="redict_to('.$r['is_blocked'].','.$r['id'].')"><i class="fa fa-check"></i></a>'.$r['title'].'</li>';
            }
        }
    ...
    public function quick_rfp_form(){
        
        $data['rfp_def_val'] = '';

        if($this->input->get('cat_id') != ''){            
            $res = $this->db->get_where('treatment_category',['id'=>$this->input->get('cat_id')])->row_array();
        ...
        if($this->form_validation->run() === FALSE){
            ...
            redirect('rfp/view_rfp/'.encode($last_id));
        }
    }
