...
//                        pr($upd_arr,1);
    ...
    public function view_bookmarked_post()
    {
        ...
        $where = ['user_id' => $sess_data['id']];
        $config['base_url'] = base_url().'dashboard/view_bookmarked_post';
        $config['total_rows'] = $this->Post_model->get_posts_front_count($where);
        $config['per_page'] = 3;
        $offset = $this->input->get('per_page');
        $config = array_merge($config,pagination_front_config());
        $this->pagination->initialize($config);
        $data["bookmark"] = $this->Post_model->get_front_result('user_bookmarks',$where,$config['per_page'],$offset);
