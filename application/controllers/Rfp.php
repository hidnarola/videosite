...
class Rfp extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model(['Treatment_category_model','Rfp_model','Messageboard_model','Notification_model','Promotional_code_model','Users_model']);		
	}
	...
	public function add($step='0'){
		...
		$data['rfp_price'] = config('patient_fees');
		...
		elseif($step == 1)
		{
			...
			if($this->form_validation->run() == FALSE)
			{
				$where = 'is_deleted !=  1 and is_blocked != 0';
	...
	public function edit($id='',$step='0'){
		...
		if($rfp_arr['rfp_type'] == 'normal'){
   			$data['rfp_price'] = config('patient_fees');
   		}else{
   			$data['rfp_price'] = config('doctor_initial_fees_quick');
   		}
    ...
    public function view_rfp($rfp_id){
        ...
        if($this->session->userdata('client')['role_id'] == '4') // Check For Doctor Role (4)
        {
       	...
       	elseif($this->session->userdata('client')['role_id'] == '5') // Check For Patient Role (5)
       	{
       		...
       		if($data['record']['rfp_type'] == 'normal'){
       			$data['rfp_price'] = config('patient_fees');
       		}else{
       			$data['rfp_price'] = config('doctor_initial_fees_quick');
       		}
    ...
    */
    ...
    public function make_payment(){
		if($_POST) {
			...
			$rfp_id = decode($this->input->post('rfp_id'));
			$rfp_data=$this->Rfp_model->get_result('rfp',['id' => $rfp_id,'patient_id' => $this->session->userdata['client']['id'] ],'1');

			if($rfp_data['rfp_type']=='normal'){
				$final_price = config('patient_fees');
			}else{				
				$final_price = config('doctor_initial_fees_quick');
			}			
    ...
    public function paypal_payment(){
		...
		$form .= '<input type="hidden" name="business" value="payments@yourtoothfairy.com" />';
    ...
    public function complete_transaction(){
    	...
    	if($res)
    	{
    		...
    		if($rfp_data['rfp_type'] == 'normal'){
    			...
    			$update_status=$this->Rfp_model->update_record('rfp',$condition,['status'	=> 3 , 'is_paid' => 1]);
    		}
    ...
    public function choose_winner_doctor($rfp_id,$rfp_bid_id){
    	...
    	if($res_rfp){
	    	...
	    	if($res_rfp_bid){
		    	...
		    	//------ For Email Template -----------
	            $user_data = $this->Users_model->check_if_user_exist(['id' => $rfp_bid_fetch['doctor_id']], false, true);
	            $subject_mail = config('site_name').' - Congratulations! For '.$rfp_data['title'].', the patient selected your offer!';

	            $msg = "Congratulations! For  <b>'".$rfp_data['title']."'</b>, the patient selected your offer! Your Next Step: Confirm your Payment and Schedule an appointment.<br><br>";
	            $msg .= "Please <a href='".base_url()."dashboard?proceed_rfp=".$rfp_id."'>click here</a> to confirm your payment and schedule an appointment or simply login to your account.<br><br>";
